...
export default function MultipleSelectDemo() {
	...
	const selectedPolygonsRef = useRef<SelectedPolygon[]>([]);
			...
			Proximiio.Auth.setToken(import.meta.env.VITE_PROXIMIIO_TOKEN)
				.then(() => {
					...
					const map = new Proximiio.Map({
						...
						polygonLayers: [
							...
							},
							{
								...
								defaultPolygonColor: '#505050',
								selectedPolygonColor: '#505050',
								hoverPolygonColor: '#505050',
								disabledPolygonColor: '#505050',
								selectedLabelColor: '#ffffff',
								defaultLabelColor: '#ffffff',
							},
						],
					...
					map.getPolygonClickListener().subscribe(async (feature) => {
						...
						if (!polygon) {
							console.warn('clicked polygon not found in feature set');
							return;
						}

						const existingIndex = selectedPolygonsRef.current.findIndex(
							(entry) => entry.polygonId === polygon.id,
						);

						if (existingIndex !== -1) {
							// Already selected -> deselect and restore its original type
							const { originalType } =
								selectedPolygonsRef.current[existingIndex];
							...
							// remove from the tracked selection list
							selectedPolygonsRef.current.splice(existingIndex, 1);
						} else {
							// Not selected yet -> remember its current type, then mark it selected
							selectedPolygonsRef.current.push({
								polygonId: polygon.id,
								polygonObject: polygon,
								featureObject: feature,
								originalType: polygon.properties.type,
							});
